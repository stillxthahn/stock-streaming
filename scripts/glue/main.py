...
args = getResolvedOptions(sys.argv, ['JOB_NAME', 'catalog_database', 'catalog_table', 's3_bucket', 's3_folder'])
sc = SparkContext()
...
AmazonS3_node1739281797147 = glueContext.create_dynamic_frame.from_options(format_options={"quoteChar": "\"", "withHeader": True, "separator": ",", "multiLine": "false", "optimizePerformance": False}, connection_type="s3", format="csv", connection_options={"paths": [f"s3://{args['s3_bucket']}/{args['s3_folder']}/"]}, transformation_ctx="AmazonS3_node1739281797147")
